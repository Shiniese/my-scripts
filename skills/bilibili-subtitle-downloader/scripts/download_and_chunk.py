...
# Configuration
COOKIE_FILE = os.path.expanduser('~/.picoclaw/workspace/bilibili_cookie.txt')
CHARS_PER_CHUNK = 100000 
QR_IMAGE_PATH = os.path.expanduser('~/.picoclaw/workspace/bilibili_login_qr.png')
...
def fetch_subtitle_content(bv_id, cid, cookie):
    # 修改这里：将 /x/player/v2 改为 /x/player/wbi/v2
    ...
    # 注意：wbi接口可能需要额外的签名参数，但先试试直接改URL
    params = {'aid': None, 'cid': cid, 'bvid': bv_id}  # 建议同时传aid
...
async def main():
    if len(sys.argv) < 2:
        print("Usage: uv run download_and_chunk.py <BV_ID> [P_NUM]")
        sys.exit(1)
